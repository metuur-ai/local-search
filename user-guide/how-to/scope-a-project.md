...
Wrote /path/to/project/.agents/local-search-config.yaml with repositories = [myrepo] (auto-detected from cwd-walk (myrepo))
```
...
Wrote /path/to/project/.agents/local-search-config.yaml with repositories = [squirrel uncle-os]
```
...
{
  "path": "/path/to/project/.agents/local-search-config.yaml",
  "exists": true,
  "empty": false,
...
Cleared repositories in /path/to/project/.agents/local-search-config.yaml (weights and limits kept).
...
/path/to/project/.agents/local-search-config.yaml:4:1: unknown key "repositorys"
   4 | repositorys:
     | ^
...
[dry-run] migrated /path/to/project/.local-search.toml → /path/to/project/.agents/local-search-config.yaml
