...
Manages the per-project scope file `<project>/.agents/local-search-config.yaml`, which
...
| Path | Contents |
|---|---|
| `<project>/.agents/local-search-config.yaml` | Per-project search scope (`repositories:` list). Managed by `local-search init`/`setup`; read by the skill, which passes the list to `local-search search` via `--repos` (and to `find`/`code` via `--scope`). |
